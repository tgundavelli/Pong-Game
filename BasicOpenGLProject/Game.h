...
public:
	...
	bool Collision(float Ball_posX, float Ball_posY, float Paddle_posX, float Paddle_posY) {
		...
		float distance2 = abs(Ball_posY - (Paddle_posY + 0.3f));
		float distance3 = abs(Ball_posX - (Paddle_posX + 0.04f));
		...
		if (Ball_posX > (Paddle_posX + 0.04f)){ //circle is right of the right side of rectangle
			d1 = distance3;
		}
		if (Ball_posY > (Paddle_posY + 0.3f)) { //circle is above of the top side of rectangle
			d2 = distance2;
		}
